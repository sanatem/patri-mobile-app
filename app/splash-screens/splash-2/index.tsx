import React from 'react';
import { View, Text, StyleSheet } from 'react-native';
import { PatrimoreIcon, LineChartIcon } from '@/components/icons';
import Colors from '@/constants/Colors';

interface SplashScreen2Props {
  title: string;
  subtitle: string;
}

export default function SplashScreen2({ title, subtitle }: SplashScreen2Props) {
  return (
    <View style={styles.container}>
      <View style={styles.logoContainer}>
        <PatrimoreIcon width={120} height={60} color={Colors.secondary[500]} />
      </View>
      <View style={styles.iconContainer} className="rounded-full">
        <LineChartIcon width={60} height={60} />
      </View>
      <Text style={styles.title} className="font-medium">
        {title}
      </Text>
      <Text style={styles.subtitle} className="font-regular text-center">
        {subtitle}
      </Text>
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    paddingHorizontal: 40,
    paddingTop: 60,
  },
  logoContainer: {
    position: 'absolute',
    top: 100,
    alignItems: 'center',
  },
  iconContainer: {
    width: 80,
    height: 80,
    backgroundColor: Colors.gray[50],
    justifyContent: 'center',
    alignItems: 'center',
    marginBottom: 40,
    borderRadius: 4,
  },
  title: {
    fontSize: 28,
    color: Colors.primary[700],
    textAlign: 'center',
    marginBottom: 20,
  },
  subtitle: {
    fontSize: 16,
    color: Colors.primary[500],
    textAlign: 'left',
    lineHeight: 24,
  },
}); 